...
class simonsaysApp:
    ...
    def move(self, x=0, y=0):
        ...
        if (self.level == 0):
            self.wipescreen()
            self.touchGrid[self.convert(self.incorrect_touch[0],self.incorrect_touch[1])] = (0,0,0)
            self.pattern = []
            self.correct_touch = 0
            self.display_pattern = 1
            self.curr_count = 0
        ...
        # during display pattern, the user watches the pattern and tries to memorize it. user inputs are not accepted
            ...
            if (self.blink_square):
                self.touchGrid[self.convert(self.pattern[self.curr_count][0],self.pattern[self.curr_count][1])] = (255,255,255)
                self.blink_square = 0
            # turns the previous square in the pattern back to black
            else:
                self.touchGrid[self.convert(self.pattern[self.curr_count][0],self.pattern[self.curr_count][1])] = (0,0,0)
                self.curr_count += 1
                self.blink_square = 1
    ...
    def paint(self, x, y):
        if (x == self.pattern[self.curr_count][0] and y == self.pattern[self.curr_count][1]):
            ...
            self.touchGrid[self.convert(self.pattern[self.curr_count][0],self.pattern[self.curr_count][1])] = (0,255,0)
