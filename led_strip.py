...
class led_strip:
    def __init__(self):
        ...
        for i in range(self.num_pixels):
            self.turn_on_led(self.strip, i, Color(0,0,0))
        self.setup_painting(self.strip)
    ...
    def setup_painting(self):
        for i in range(8):
            n = self.convert(i, 15)
            self.turn_on_led(self.strip, n, Color(setColors[i][0], setColors[i][1], setColors[i][2]))
            self.touch_array[n] = self.rgbToHex(setColors[i][0], setColors[i][1], setColors[i][2])
        self.turn_on_led(self.strip, self.convert(8,15), Color(0, 0, 255))
        self.touch_array[self.convert(8, 15)] = self.rgbToHex(0, 0, 255)
        self.turn_on_led(self.strip, self.convert(9,15), Color(0, 255, 0))
        self.touch_array[self.convert(9, 15)] = self.rgbToHex(0, 255, 0)
        self.turn_on_led(self.strip, self.convert(10,15), Color(255, 0, 0))
        self.touch_array[self.convert(10, 15)] = self.rgbToHex(255, 0, 0)
        self.turn_on_led(self.strip, self.convert(11,15), self.stored_color)
